...
def crawlingAllA(url):
  ...
  for i in range(16):
    ...
    for a in pageA:
      ...
      # 爬这个帖子的详情
      postContent = crawlingDetail(postBar + href)
     # sleep(5)
      s = s + 1
      keywords = tfidf(postContent)

      keyWord = ""
      for keyword in keywords:
        keyWord += keyword + ","
      postContent = '关键词：' + keyWord + '\n' + postContent
      string = re.sub("[\s+\.\!\/_,$%^*(+\"\')]+|[+——()?【】“”！，。？、~@#￥%……&*（）]+'", "", tittle)
      try:
        f = open(tittle + '.txt', 'w+', encoding='utf-8')
        f.writelines(postContent)
      except IOError:
